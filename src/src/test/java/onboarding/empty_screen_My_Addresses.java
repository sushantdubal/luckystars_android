...
public class empty_screen_My_Addresses extends base{
	


	@Test(priority = 1, groups = "Sign up")
	public void empty_screen_My_Addresses() throws InterruptedException 
	{
		try 
		 {
			//call setup method
		...
		driver.findElement(By.name("My Addresses")).click();
		
		
		
		if (driver.findElements(By.id("tv_no_order")).size() == 1)
		{
			driver.findElement(By.id("tv_no_order")).click();
