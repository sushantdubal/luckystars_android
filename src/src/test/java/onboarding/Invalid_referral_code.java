...
import config.base;
public class Invalid_referral_code extends base
{
	
	@Test(priority = 1, groups = "Sign up")
		public void Invalid_referral_code() throws InterruptedException 
		{
			try 
			 {
				//call setup method
			...
			//Enters email id
			driver.findElement(By.id("et_email_id")).sendKeys("sushant@init.in");

			//Hide Keyboard
			driver.navigate().back();

			driver.findElement(By.id("et_ref_code")).sendKeys("abc");
			...
			if (driver.findElements(By.name("Sorry, this is an invalid reference code. Please check and re-enter.")).size() == 1)
